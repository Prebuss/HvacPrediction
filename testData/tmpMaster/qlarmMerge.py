...
def merge_csv_to_daily_csv(input_folder):
    """
    For each day (1..28), gather CSV data from all sensors
    and write a single 'qlarmData 2025.02.dd.csv' file.
    """
    ...
    for day in range(1, 29):
        ...
        for sensor in SENSORS:
            ...
            file_pattern = os.path.join(
                input_folder,
                f"{sensor_name} 2025.02.{day:02d}.csv"
            )
            
            matched_files = glob.glob(file_pattern)
            
            for csv_file_path in matched_files:
                with open(csv_file_path, "r", encoding="utf-8") as csv_file_handle:
                    lines = csv_file_handle.read().splitlines()
