...
The simulation runs multiple episodes (each = 1 day) using the exact simulation
dynamics (with minor adaptations) and saves the Q-table to disk ("hvacRL1.pkl")
so that learning persists across script restarts.
Auto-tuning methods are included to boost exploration or reset hyperparameters
if performance stalls. The reward function penalizes high temperatures (above 30°C)
and rewards temperatures near 22°C.
"""
...
# Transmission parameters (conduction)
U_yttervegg = 0.22
U_tak = 0.18
U_gulv = 0.18
U_doer = 1.2
...
# Internal gains
P_arealspes_utstyrsvarme = 8   # [W/m²]
P_arealspes_belysningsvarme = 7  # [W/m²]
P_arealspes_personvarme = 60     # [W/m²]
A_person = 1.8
...
# (Here we choose a 60-second timestep so that one episode = 24 hours.
#  Note: the original simulator used ts=2 s for a 1-hour run. Adjusting ts here
#  preserves a full-day simulation while keeping the dynamics similar.)
ts = 60                      # Time step [s]
t_stop_min = 1440            # 1440 minutes = 1 day
N_sim = int((t_stop_min * 60) / ts)  # number of timesteps per episode

# Delay arrays for room and heat battery (will be reset each episode)
td_rom = 60               # [s]
...
# ---------------------
# 1. RL Agent with Adjustable Hyperparameters and Auto-Tuning (Q-table based)
# ---------------------
...
def compute_reward(T_new, T_old, occupancy, u, u_prev, energy_avg, gamma=0.95, lambda_phi=0.1):
    """
    Computes a reward that incentivizes maintaining the target temperature
    (22°C when occupied, 18°C when not) and penalizes high temperatures (above 30°C).
    A bonus is provided if the temperature is within ±1°C of the target.
    """
    target = 22 if occupancy > 0 else 18
    error = abs(T_new - target)
    bonus = 50000 if error <= 1 else 0
    deviation_penalty = 100 * error
    control_penalty = 1 * abs(u - u_prev)
    energy_penalty = 1 * u
    high_temp_penalty = 20 * (T_new - 30) if T_new > 30 else 0
...
# ---------------------
# 4. PID Parameters and Function for the Heat Battery (TC2)
# ---------------------
tcc_vb = td_vb  # time constant factor
...
def fun_pid_controller(r_k, y_k, u_man, ui_km1, pid_par, u_par, ts):
    (Kc, Ti, _) = pid_par  # Use the computed PID parameters
    (u_max, u_min) = u_par
    e_k = r_k - y_k
    up_k = Kc * e_k
...
def run_episode(episode, rl_agent):
    # Reset delay arrays at the start of each episode
    global delay_array_rom, delay_array_vb
    delay_array_rom[:] = 0
    delay_array_vb[:] = 0

    # Initial conditions
    T_rom_k = 18.0  # initial room temperature
    T_ute_init = -1  # initial outdoor temperature baseline
    # Initial heat recovery temperature (based on outdoor and target room temperature)
    T_vg_prim_ut_init = T_ute_init + virkgrad_vg * (22 - T_ute_init)
    T_vb_ut_k = T_vg_prim_ut_init  # initial heat battery outlet temperature
    ui_TC2_km1 = 0
    ...
    # Arrays to record simulation data for plotting
    ...
    for k in range(N_sim):
        # Limit room temperature to physical bounds
        T_rom_k = np.clip(T_rom_k, -20, 100)
        t_k = k * ts
        t_array[k] = t_k
        hour = (t_k / 3600) % 24

        # Outdoor temperature follows a sinusoidal pattern
        T_ute_k = T_ute_init + 5 * np.sin(2 * np.pi * (hour - 6) / 24)
        T_ute_array[k] = T_ute_k

        # Update occupancy once per hour
        if int(t_k) % 3600 == 0:
            if 7 <= hour < 18:
                if int(hour) == 7:
                    occupancy_current = np.random.randint(1, 3)
                else:
                    delta = 0#np.random.randint(0, 1)
        ...
        # Set control setpoints (room and battery)
        r_TC1_k = 22  # target room temperature
        r_TC2_k = 19  # target for ventilation inlet temperature

        # Compute transmission losses/gains
        ...
        # Solar gains (using a simple model between 10 and 16 hours)
        if 10 <= hour <= 16:
        ...
        # Internal gains (lighting and equipment active during daytime)
        if 7 <= hour < 18:
            belysning_on_off = 1
            utstyr_on_off = 1
        else:
        ...
        # Heat battery dynamics (TC2) via PID controller
        (u_TC2_k, ui_TC2_k) = fun_pid_controller(r_TC2_k, T_tilluft_k, 0, ui_TC2_km1,
                                                  pid_TC2_par, u_TC2_par, ts)
        u_TC2_array[k] = u_TC2_k

        # Introduce delay in the heat battery control signal
        P_vb_delayed = delay_array_vb[-1]
        delay_array_vb[1:] = delay_array_vb[:-1]
        delay_array_vb[0] = u_TC2_k

        # Energy balance for the heat battery (TC2)
        ...
        # --- Room control: Use RL agent action in place of PID controller ---
        state = discretize_state(T_rom_k, n_personer_k, t_k)
        action = rl_agent.choose_action(state)
        u_TC1_k = action  # RL-controlled heating power (in Watts)
        u_TC1_array[k] = u_TC1_k

        # Update energy history for averaging
        u_history.append(u_TC1_k)
        if len(u_history) > 60:
            u_history.pop(0)
        energy_avg = np.mean(u_history) if u_history else 0

        # Introduce delay in the room control signal
        P_rom_delayed = delay_array_rom[-1]
        delay_array_rom[1:] = delay_array_rom[:-1]
        delay_array_rom[0] = u_TC1_k

        # Energy balance for the room (TC1)
        ...
        # Compute reward based on the new room temperature, setpoint, and control smoothness
...
# ---------------------
# 6. Continuous Loop with Auto-Tuning and Persistent Q-Table
# ---------------------
# Define the set of discrete actions (heating power in Watts)
actions = list(range(0, 5001, 100))
qtable_filename = "hvacRL3.pkl"

# Load an existing Q-table if available, otherwise initialize a new RL agent
...
try:
    while True:
        ...
        # Auto-tuning: if the cumulative reward does not improve, boost epsilon or reset hyperparameters
