...
###########################################
# 5) BUILD MODEL (LSTM/GRU + static branch)
###########################################
def build_sequence_model(hp):
    """
    Build either LSTM or GRU with a parallel static input branch.
    Uses 'loss=mae' as requested.
    """
    rnn_type = hp.Choice('rnn_type', values=['LSTM', 'GRU'], default='LSTM')
    num_rnn_layers = hp.Int('num_rnn_layers', 1, 4, default=1)
    units_1 = hp.Int('units_1', 32, 512, step=32, default=64)
    dropout_1 = hp.Float('dropout_1', 0.0, 0.7, step=0.1, default=0.2)
    rec_dropout_1 = hp.Float('rec_dropout_1', 0.0, 0.7, step=0.1, default=0.2)
...
###########################################
# 6) (Optional) PLOT HYPERPARAMETERS
###########################################
def plot_hyperparams(tuner):
    """Plot each hyperparameter vs final val_loss (optional)."""
    import matplotlib.pyplot as plt

    trials = tuner.oracle.trials.values()
    if not trials:
        print("No trials found for plotting hyperparams.")
        return

    hp_names = list(trials)[0].hyperparameters.values.keys()

    plt.figure(figsize=(6 * len(hp_names), 4))
    for i, hp_name in enumerate(hp_names, start=1):
        hp_vals, losses = [], []
        for t in trials:
            if t.score is not None:
                val = t.hyperparameters.values.get(hp_name)
                loss = t.score
                hp_vals.append(val)
                losses.append(loss)
        plt.subplot(1, len(hp_names), i)
...
def main():
    ...
    # 8) Bayesian Optimization with Keras Tuner
    ...
    # Optionally plot hyperparameters
    # plot_hyperparams(tuner)

    # 9) Get best model and evaluate on test
    ...
    if best_models:
        ...
        print("No best model found. Possibly no successful trials?")
