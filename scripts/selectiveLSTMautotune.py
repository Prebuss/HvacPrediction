...
def create_sequences(data, target_column, seq_length):
    ...
    for i in range(len(data) - seq_length):
        ...
        # Only append sequences with no NaNs in inputs or the target
...
# ======= AUTOMATIC HYPERPARAMETER TUNING (LSTM) ======= #
def build_model(hp):
    model = Sequential()
    
    # Tune the number of LSTM layers (1 or 2)
    ...
    if num_lstm_layers > 1:
        # When using two LSTM layers, the first must return sequences.
        model.add(LSTM(
            units_1,
...
# Set up the Keras Tuner (replace GRU references with LSTM references)
tuner = kt.RandomSearch(
    ...
    project_name='lstm_hvac_tuning'  # Renamed project
)
...
# Save the best model (renamed for LSTM)
best_model.save("lstm_hvac_forecast_best.keras")
print("✅ Best model saved as 'lstm_hvac_forecast_best.keras'.")

# Optionally, plot the training loss of the best model (if available)
# (Not all Keras Tuner models keep a history accessible in best_model.history)
if hasattr(best_model, 'history') and best_model.history is not None:
